...
export function Confidentiality() {
  ...
  return (
    <MotionSection
      ...
      className="relative overflow-hidden bg-[#0b0e0c] py-16 md:py-44"
    >
      ...
      <div className="section-shell">
        <div className="premium-card relative overflow-hidden rounded-md p-5 shadow-premium md:p-14 lg:p-16">
          ...
          <div className="relative grid gap-10 lg:grid-cols-[1fr_0.72fr] lg:items-end">
            <div className="max-w-4xl">
              ...
              <h2 className="mt-3 text-[2.02rem] font-semibold leading-[1.07] text-[#f8f8f4] min-[390px]:text-[2.18rem] md:mt-5 md:text-6xl">
                {t.confidentiality.title}
              </h2>
              <p className="mt-8 max-w-2xl text-lg leading-9 text-stone/68">
                {t.confidentiality.text}
              </p>
              <div className="mt-6 grid gap-2 text-sm text-stone/66 md:mt-12 md:grid-cols-3 md:gap-3">
                {t.confidentiality.badges.map((badge) => (
                  <span key={badge} className="rounded-md border border-stone/10 bg-white/[0.035] p-4 transition hover:border-fern/30 hover:text-[#f8f8f4] md:p-5">
                    {badge}
                  </span>
                ))}
              </div>
