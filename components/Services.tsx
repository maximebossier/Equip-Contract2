    ...
    >
      <div className="section-shell grid gap-10 md:gap-14 lg:grid-cols-[0.88fr_1fr] lg:items-start">
        <div className="lg:sticky lg:top-28">
          ...
          <div className="mt-8 overflow-hidden rounded-md border border-stone/12 bg-[#0f130f] shadow-premium md:mt-12">
            <div className="relative h-64 md:h-72">
              ...
              <div className="absolute inset-x-5 bottom-5 md:inset-x-6 md:bottom-6">
                ...
                <p className="mt-3 max-w-sm text-xl font-semibold leading-tight text-[#f8f8f4] md:text-2xl">
        ...
        <div className="grid gap-4">
          {t.services.items.map((service, index) => (
            <MotionArticle
              ...
              className="premium-card group rounded-md p-5 transition duration-500 hover:-translate-y-1 hover:border-fern/35 hover:bg-white/[0.06] md:p-6"
            >
              <div className="flex items-start justify-between gap-6">
                <div>
                  <span className="text-xs font-bold uppercase tracking-[0.18em] text-fern">{String(index + 1).padStart(2, "0")}</span>
                  <h3 className="mt-3 text-xl font-semibold text-[#f8f8f4] transition group-hover:text-fern md:mt-4 md:text-2xl">{service.title}</h3>
                </div>
